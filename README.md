...
├── astro.config.mjs               # Astro 配置（插件、站点 URL）
├── public/
...
│   ├── components/
│   │   ├── BaseHead.astro         # <head> 公共标签，含 llms.txt 发现入口
│   │   ├── Header.astro           # 导航栏
│   │   ├── Footer.astro           # 页脚
│   │   ├── Giscus.astro           # 评论组件
...
│   ├── layouts/
│   │   └── BlogPost.astro         # 文章页布局（集成评论、阅读时间等）
│   ├── pages/
│   │   ├── index.astro            # 首页
│   │   ├── about.astro            # 关于页
│   │   ├── blog/
...
│   ├── styles/
│   │   └── global.css             # 全局样式（含暗色模式、wikilink 样式）
...
推送到 `main` 分支后自动触发：`npm ci` → `npm run build` → 部署 `dist/` 到 GitHub Pages。
...
5. **暗色模式**：通过 `prefers-color-scheme` 自动切换，无手动开关
